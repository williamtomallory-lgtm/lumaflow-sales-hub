...
**电脑执行边界：**Chat 只回答或生成内容；Work 仅在本机网站通过已授权的本机 CowAgent 执行命令、读写文件，并显示真实工具回执。公共 Vercel 页面不能直接访问个人电脑，也不接受公开网页发来的电脑执行请求。微信命令由本机 CowAgent 的独立微信会话授权控制。要让远程网页安全执行本机命令，还需账号认证、设备绑定与加密出站连接，不能只公开一个端口或 API 密钥。
