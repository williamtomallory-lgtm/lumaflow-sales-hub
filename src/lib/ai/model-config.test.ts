...
import { getModelConfig, getModelHealth, getModelOptions, LOCAL_MODEL_ID } from "./model-config";
...
describe("allowlisted model profiles", () => {
  ...
  it("publishes only display metadata and never the service URL or key", async () => {
    vi.stubGlobal("fetch", vi.fn(async (url: string) => Response.json({
      data: [{ id: url.includes("11434") ? LOCAL_MODEL_ID : "custom-model" }],
    })));
    ...
    expect(payload.data.models).toHaveLength(2);
  ...
  it("keeps the local model selectable when an optional custom profile is invalid", async () => {
    ...
    expect(models[1]).toMatchObject({ id: "configured", reachable: false, configured: false });
  });
  ...
  it("serves the model catalog with the local profile as the UI default", async () => {
    ...
    expect(payload.data.models.map((item) => item.id)).toEqual(["local-qwen3-8b", "configured"]);
  });
});
